...
SIDECARS=(buzz-acp buzz-agent buzz-dev-mcp git-credential-nostr buzz buzz-backend-wyzor-hermes)
...
if [[ ${#missing[@]} -gt 0 ]]; then
    ...
    echo "Run 'cargo build --release -p buzz-acp -p buzz-agent -p buzz-dev-mcp -p git-credential-nostr -p buzz-cli -p wyzor-hermes-provider' first." >&2
    exit 1
fi
